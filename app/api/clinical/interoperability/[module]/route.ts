...
export async function GET(
  ...
  const config =
    getInteropModuleConfig(module);

  if (!config) {
    return NextResponse.json(
      { error: "Unknown interoperability module." },
      { status: 404 }
    );
  }
  ...
  const [rows, screens, reports, endpoints, metrics] =
    await Promise.all([
      ...
      prisma.$queryRawUnsafe<Row[]>(
        ...
        FROM clinical_interop_api_endpoint_definitions
        WHERE tenant_id = $1
          AND hospital_id = $2
          AND branch_id = $3
          AND module_key = $4
...
export async function POST(
  ...
  await recordClinicalAudit(context, {
    moduleName: `interop_${config.key}`,
    action: "create",
  ...
  await prisma.$executeRawUnsafe(
    `
    INSERT INTO clinical_interop_timeline (
      tenant_id,
      hospital_id,
      branch_id,
      clinic_id,
    ...
    `${config.label} interoperability workflow record created`,
    config.table,
    Number(row.id),
    JSON.stringify(row),
    context.user.id ?? null
...
export async function PATCH(
  ...
  if (!config?.statusColumn) {
    return NextResponse.json(
      {
        error:
          "This interoperability module does not support status updates.",
      },
      { status: 400 }
    );
  }
  ...
  const status = normalizeInteropValue(
    config,
    config.statusColumn,
    body.status
  );
