...
import { getBusinessSpecModuleConfig } from "@/lib/clinical/business-spec-core";
...
export async function GET(
  ...
  const config =
    getBusinessSpecModuleConfig(module);

  if (!config) {
    return NextResponse.json(
      {
        error:
          "Unknown clinical business specification module.",
      },
      { status: 404 }
    );
  }
