...
import {
  getApiCatalogModuleConfig,
  type ApiCatalogModuleConfig,
} from "@/lib/clinical/api-catalog-core";

type Row = Record<string, unknown>;

type ModulePayload = {
  module?: ApiCatalogModuleConfig;
...
const moduleIcons: Record<string, LucideIcon> = {
  ...
  "websocket-events": RadioTower,
  events: Workflow,
  rabbitmq: RadioTower,
  webhooks: Globe2,
  errors: AlertTriangle,
  versioning: GitBranch,
  "rate-limits": ShieldCheck,
  openapi: FileText,
  integrations: Globe2,
};

export default function ClinicalApiCatalogModulePage() {
  const params =
    useParams<{ module: string }>();
  const moduleKey = params?.module || "";
  const config = useMemo(
    () => getApiCatalogModuleConfig(moduleKey),
    [moduleKey]
  );
  const [data, setData] =
  ...
  const load = useCallback(async () => {
    ...
    const response = await fetch(
      `/api/clinical/api-catalog/${moduleKey}`
    );
        ...
        <div className="p-4">
          <section className="rounded-[8px] border border-red-200 bg-white p-6 shadow-sm">
            <h1 className="text-3xl font-black">
              Unknown API Catalog Module
            </h1>
            <p className="mt-2 text-sm font-semibold text-slate-600">
              This Phase 14 API catalog module is not registered.
            </p>
          </section>
        </div>
      </ClinicalShell>
  ...
  return (
    <ClinicalShell>
      <div className="space-y-6 p-4">
        <Link
          href="/clinical-services/api-catalog"
...
function ApiCatalogRow({
  ...
  return (
    <ClinicalRecordCard
      href={`/clinical-services/api-catalog/${moduleKey}/${row.id}`}
      eyebrow={`ID ${String(row.id || "-")}`}
      title={formatValue(row[titleColumn])}
      description="API contract record"
      status={String(row.status || row.method || row.event_type || "-")}
      editHref={`/clinical-services/api-catalog/${moduleKey}?record=${row.id}&mode=edit`}
      auditHref={`/clinical-services/api-catalog/${moduleKey}/${row.id}#audit`}
      historyHref={`/clinical-services/api-catalog/${moduleKey}/${row.id}#history`}
    >
