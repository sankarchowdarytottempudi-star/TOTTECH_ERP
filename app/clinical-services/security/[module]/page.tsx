...
import {
  getSecurityModuleConfig,
  type SecurityModuleConfig,
} from "@/lib/clinical/security-core";

type Row = Record<string, unknown>;

type ModulePayload = {
  module?: SecurityModuleConfig;
...
const moduleIcons: Record<string, LucideIcon> = {
  ...
  "user-permissions": UserCheck,
  "data-masks": EyeOff,
  "record-policies": ShieldCheck,
  "field-policies": EyeOff,
  "export-controls": FileText,
  "bulk-actions": Workflow,
  "approval-workflows": Workflow,
  "workflow-steps": Workflow,
  mfa: LockKeyhole,
  "session-security": LockKeyhole,
  "access-logs": FileText,
  "security-events": ShieldCheck,
  "break-glass": ShieldCheck,
  reports: FileText,
  "api-groups": Database,
};

export default function ClinicalSecurityModulePage() {
  const params =
    useParams<{ module: string }>();
  const moduleKey = params?.module || "";
  const config = useMemo(
    () => getSecurityModuleConfig(moduleKey),
    [moduleKey]
  );
  const [data, setData] =
  ...
  const load = useCallback(async () => {
    ...
    const response = await fetch(
      `/api/clinical/security/${moduleKey}`
    );
        ...
        <div className="p-4">
          <section className="rounded-[8px] border border-red-200 bg-white p-6 shadow-sm">
            <h1 className="text-3xl font-black">
              Unknown Security Module
            </h1>
            <p className="mt-2 text-sm font-semibold text-slate-600">
              This Phase 13 security module is not registered.
            </p>
          </section>
        </div>
      </ClinicalShell>
          ...
          <div className="mt-5 grid gap-4">
            {rows.length ? (
              ...
              <p className="rounded-[8px] border border-slate-200 bg-slate-50 p-4 text-sm font-bold text-slate-600">
                No security records found for this module.
              </p>
            )}
          </div>
        </section>
...
function SecurityRow({
  ...
  return (
    <ClinicalRecordCard
      href={`/clinical-services/security/${moduleKey}/${row.id}`}
      eyebrow={`ID ${String(row.id || "-")}`}
      title={formatValue(row[titleColumn])}
      description="Security governance record"
      status={String(row.status || row.access_status || row.event_type || "-")}
      editHref={`/clinical-services/security/${moduleKey}?record=${row.id}&mode=edit`}
      auditHref={`/clinical-services/security/${moduleKey}/${row.id}#audit`}
      historyHref={`/clinical-services/security/${moduleKey}/${row.id}#history`}
    >
